...
| **Aster runtime kind (v1.1+, preview)** | deployments can be created with `kind: "aster"` to register an [Aster runner cell](https://github.com/Iann29/aster) — capability-narrowed execution plane that runs tenant JS in a V8 cell with **no database credentials**. Synapse provisions the brokerd container and can spawn a raw-JS v8cell via `POST /v1/deployments/{name}/aster/invoke`; the deployment proxy still returns typed `501 aster_not_proxied` until the Convex-shaped HTTP frontend lands. The dashboard renders an amber "aster" badge. Aster's Postgres adapter, Convex `db.get` syscall, IDv6 mapping, ConvexValue codec, module index, and local module-storage adapter are wired; the remaining end-to-end work is the cell-side module loader, shared Postgres/modules wiring, and HTTP frontend — see [`docs/ASTER_INTEGRATION.md`](docs/ASTER_INTEGRATION.md). |
