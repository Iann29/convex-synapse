...
- [x] **v0.6.0 — Foundation ✅ DONE.** `./setup.sh` script + supporting compose changes. **Validated end-to-end against a real Hetzner CPX22** (Ubuntu 24.04). One-line `git clone && ./setup.sh --domain=<host>` produces a working install in ~3 min cold.
  ...
  - [ ] **Bookmark — "Open dashboard" button standalone-host wiring.** Today the dashboard fork's "Open dashboard" link points at `http://localhost:6791/...` which only works if the operator runs the standalone Convex dashboard locally. Need to either (a) host the standalone dashboard alongside Synapse via the compose stack and route it through `<PublicURL>/dashboard/...`, or (b) make the host configurable via a dashboard env var. Defer to v0.6.3 (browser-driven first-run wizard) since it's UX glue, not core function
...
- [ ] **v0.6.3 — Browser-driven first-run wizard + dashboard polish.** Dashboard's `/login` redirects to `/setup` when no users exist; walks the operator through admin-create → optional HA → demo deployment + CLI snippet. Operator never sees a config file. **Also includes the "Open dashboard" standalone-host fix** carried over from v0.6.0: today the link points at `http://localhost:6791/...` which only works if the operator runs the standalone Convex dashboard locally. Either host the standalone alongside Synapse via the compose stack and route through `<PublicURL>/dashboard/...`, or expose a `NEXT_PUBLIC_CONVEX_DASHBOARD_URL` env var the dashboard fork respects.
