...
| `synapse/internal/db/migrations/` | `go:embed`'d SQL migrations applied at startup. v1.0+: 8 migrations (init, jobs, adopted, replicas, replica_id on jobs, upgrade_to_ha kind, app token scope, project_members RBAC) |
...
| `installer/install/` | Phase scripts the orchestrator composes (preflight, secrets, caddy, compose, verify, ui) + `lifecycle.sh` (upgrade / backup / restore / uninstall / logs / status) + `wizard.sh` (interactive Q&A walkthrough fired when no mode flags passed) |
