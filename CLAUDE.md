...
| **`--upgrade` (v0.6.1 chunk 1)**: `setup.sh --upgrade [--ref=<git-ref>] [--force]` clones the target ref into a temp dir, rsync-syncs into the install dir (preserving .env / Caddyfile / upgrade.log / .upgrade-snapshot.tsv), pre-pulls external images, `compose up -d --build` with the original profile flags, waits `/health`, on failure re-tags from the snapshot and brings the stack back up. Auto-detects target via the GitHub Releases /latest API (falls back to `main`). Stamp lives in `.env` SYNAPSE_VERSION (slashes sanitized → '-'); audit trail in `$INSTALL_DIR/upgrade.log`. Real-VPS validated end-to-end | `installer/install/lifecycle.sh`, `setup.sh::main UPGRADE branch` |
